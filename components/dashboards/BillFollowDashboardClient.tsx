...
export function BillFollowDashboardClient({
  ...
  const yearVatRows = useMemo(() => filterRowsByYear(vatRows), [vatRows, filterRowsByYear]);
  const yearCreditRows = useMemo(() => filterRowsByYear(creditRows || []), [creditRows, filterRowsByYear]);

  // VAT Aging buckets (คำนวณจำนวนวันค้างส่งบิล VAT)
  const urgentVatRows = useMemo(() => {
    return yearVatRows.filter((r) => calculateDaysElapsed(r["ว/ด/ป"]) >= 15);
  }, [yearVatRows]);

  const warningVatRows = useMemo(() => {
    return yearVatRows.filter((r) => {
  ...
  const normalVatRows = useMemo(() => {
    return yearVatRows.filter((r) => calculateDaysElapsed(r["ว/ด/ป"]) < 8);
  }, [yearVatRows]);

  // ตามบิล แสดงเฉพาะบิลที่มี VAT ที่ค้างส่งใบกำกับภาษี/ใบเสร็จ (หัก ณ ที่จ่าย จะไม่แสดงในหน้านี้)
  const allPendingRows = useMemo(() => yearVatRows, [yearVatRows]);
  ...
  const activeCategoryRows = useMemo(() => {
    switch (activeTab) {
      ...
      case "credit":
        return yearCreditRows;
      default:
        return allPendingRows;
    }
  }, [activeTab, allPendingRows, urgentVatRows, warningVatRows, normalVatRows, yearCreditRows]);
