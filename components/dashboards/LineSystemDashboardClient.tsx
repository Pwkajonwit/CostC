...
export function LineSystemDashboardClient() {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto flex flex-col gap-3 font-sans text-slate-800 antialiased pb-12">
      {/* ========================================================================= */}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-xs sm:text-sm font-black text-slate-900 tracking-tight uppercase flex items-center gap-1.5">
                  ตั้งค่าระบบ LINE Bot & อัตโนมัติ
                ...
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[11px] font-bold bg-[#0b3531] text-[#d4f54e] border border-[#144d47] shrink-0 shadow-2xs">
                  <Sparkles className="w-3 h-3 text-[#d4f54e]" />
                  <span>LINE V2.0 Active</span>
                </span>
                <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[11px] font-bold border shrink-0 ${
                  ...
                  <span>แหล่งข้อมูล: {configSource === "supabase" ? "Supabase DB" : "Environment"}</span>
                </span>
              </div>
              <p className="text-[11px] text-slate-500 font-medium mt-0.5 truncate">
                จัดการ Token, LIFF ID, เวลาส่งสรุปงานประจำวัน, คู่มือคำสั่ง 63 คีย์เวิร์ด และตรวจสอบสถานะ Error Logs
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            >
              ...
              <span>{savingConfig ? "กำลังบันทึก..." : "บันทึกคอนฟิก LINE"}</span>
            </button>
          </div>
        </div>
      </section>
      ...
      {/* ========================================================================= */}
      {/* 2. LINE OA PACKAGE & MESSAGE QUOTA STATUS BANNER                          */}
      {/* ========================================================================= */}
      <div className="bg-white rounded-xl border border-slate-200/90 p-4 sm:p-5 shadow-2xs space-y-4 font-sans">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
          <div className="flex items-center gap-3">
            {quotaInfo?.botInfo?.pictureUrl ? (
              ...
              <div className="w-10 h-10 rounded-full bg-emerald-100 text-emerald-800 font-bold flex items-center justify-center text-xs border border-emerald-200 shadow-2xs">
                LINE
              </div>
            )}
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {quotaInfo?.botInfo?.basicId && (
                  <span className="font-mono text-xs font-bold bg-[#0b3531] text-[#d4f54e] border border-[#144d47] px-2 py-0.5 rounded-md shadow-2xs">
                    {quotaInfo.botInfo.basicId}
                  </span>
                )}
          ...
          <button
            ...
            className="self-start sm:self-auto px-3.5 py-1.5 rounded-lg bg-white hover:bg-slate-50 text-slate-700 text-xs font-bold border border-slate-200 shadow-2xs transition flex items-center gap-1.5 cursor-pointer"
          >
            <RefreshCw size={13} className={loadingQuota ? "animate-spin text-emerald-600" : "text-slate-500"} />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {/* Card 1: Package Type */}
          <div className="bg-slate-50/70 p-3 rounded-xl border border-slate-200/80 shadow-2xs flex flex-col justify-between space-y-1">
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1">
        ...
        >
          <Key size={14} className={activeTab === "config" ? "text-[#d4f54e]" : "text-emerald-700"} />
        ...
        >
          <Clock size={14} className={activeTab === "schedules" ? "text-[#d4f54e]" : "text-amber-600"} />
        ...
        >
          <BookOpen size={14} className={activeTab === "manual" ? "text-[#d4f54e]" : "text-blue-600"} />
        ...
        >
          <AlertTriangle size={14} className={activeTab === "logs" ? "text-[#d4f54e]" : "text-rose-600"} />
